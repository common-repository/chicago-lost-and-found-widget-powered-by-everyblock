<?php
/*
Plugin Name: Chicago Lost and Found Widget Powered By Everyblock 
Plugin URI: http://tirespider.com/
Description: Add a Chicago Lost and Found Widget to a post or page.
Version: 2.0.2
Author: Jeff S
...
class ChicagolostWidget extends WP_Widget {

	public static $metro = "chicago";
	public static $schema = "lost-found";
	
	public function ChicagolostWidget()
	{
		$widget_ops = array('classname' => 'ChicagolostWidget', 'description' => 'Embeds a Chicago Lost and Found Widget' );
		$this->WP_Widget('ChicagolostWidget', 'Chicago Lost and Found Widget', $widget_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("ChicagolostWidget");') );

function display_chicago_lost_widget($atts) {
	$atts = extract(shortcode_atts( array('width' => '300', 'height' => '500'), $atts, 'display_chicago_lost_widget'));
	
	$code = "<iframe src=\"" . plugins_url( 'widget.php', __FILE__ ) . "?metro=" . ChicagolostWidget::$metro . "&schema=" . ChicagolostWidget::$schema . "&width=" . $width . "&height=" . $height . "&url=" . ABSPATH . "\" width=\"". $width . "\" //height=\"" . $height . "\" frameborder='0' scrolling='no'></iframe>";
	 
	return get_html_for_embed_chicago_lost(stripslashes($code));
}

function get_html_for_embed_chicago_lost($embedCode) {
	return '<div class="widget_block">' . stripslashes($embedCode) .'</div>';  
}

add_shortcode('display_chicago_lost_widget', display_chicago_lost_widget);
